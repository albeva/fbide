...
auto WxStcStyledSource::styleAt(Sci_PositionU pos) const -> ThemeCategory {
    return static_cast<ThemeCategory>(m_stc.GetStyleAt(static_cast<int>(pos)));
}

void WxStcStyledSource::getCharRange(char* buffer, Sci_PositionU pos, Sci_PositionU len) const {
...
auto WxStcStyledSource::lineFromPosition(Sci_PositionU pos) const -> Sci_Position {
    return m_stc.LineFromPosition(static_cast<int>(pos));
}

auto WxStcStyledSource::lineState(Sci_Position line) const -> FBSciLexer::LineState {
