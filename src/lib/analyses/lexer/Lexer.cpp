...
auto Lexer::identifier(const bool firstOnLine) -> Token {
    ...
    return Token { .kind = info.tokenKind, .keywordKind = info.keywordKind, .text = text };
}
...
/// Skip ASCII space/tab characters in `text` starting at `pos`.
...
/// Match `word` case-insensitively starting at `pos` in `text`, bounded by a
/// non-word character (or end of string). Advances `pos` on success.
...
/// Classify a Comment token's text as a format pragma. Accepts both `'` and
/// `REM` prefixes. Returns None if the body (after the marker) is anything
/// other than `format on` or `format off` (case-insensitive, whitespace OK).
...
void Lexer::annotateVerbatim(std::vector<Token>& tokens) {
    int depth = 0;
    std::size_t i = 0;
    while (i < tokens.size()) {
        // [i, lineEnd) contains tokens before the Newline; lineStop is one
        // past the Newline (or tokens.size() for the trailing unterminated line).
        ...
        // Determine pragma kind only when the line holds exactly one
        // significant token, and that token is a single-line Comment whose
        // body matches the pragma grammar.
